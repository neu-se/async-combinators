...
| --- | --- |
| [`fetch-resilience.ts`](./fetch-resilience.ts) | `withRetry` / `withRateLimit` / `withTimeout` over native `fetch`: recovers from transient `503`s, paces a concurrent burst under a `429` rate limit, and composes all three, verified against a local flaky server. |
